...
open class TagView: UIView {
    ...
    open override func layoutSubviews() {
        ...
        if let _ = icon.image {
            dismissView.isUserInteractionEnabled = true
            textLeft1 = marginLeft + marginLeft/2
        } else {
            dismissView.isUserInteractionEnabled = false
            textLeft1 = marginLeft
        }
        
        textLabel.frame = CGRect(x: textLeft1, y: marginTop, width: 100, height: 30)
        ...
        if let _ = icon.image {
            dismissView.isUserInteractionEnabled = true
            textLeft2 = marginLeft + Int(textLabel.frame.width ) + marginLeft
            icon.frame = CGRect(x: textLeft2, y: marginTop+4, width: 10, height: 10)
            
        } else {
            dismissView.isUserInteractionEnabled = false
            textLeft2 = marginLeft
            icon.frame = CGRect(x: textLeft2, y: marginTop+4, width: 0, height: 0)
        }
        
        
        icon.image = iconImage?.withRenderingMode(.alwaysTemplate)
        icon.tintColor = UIColor(red: 5.0/255, green: 150.0/255, blue: 1.00, alpha: 1.00)
        icon.backgroundColor = UIColor.white
        icon.layer.cornerRadius = icon.frame.size.height / 2
        icon.layer.borderColor = UIColor.white.cgColor
        icon.layer.borderWidth = 2.00
        icon.layer.masksToBounds = true
        icon.center.y = textLabel.center.y
        
        
        let tagHeight = Int(max(textLabel.frame.height,14)) + marginTop*2
        //  let tagWidth = textLeft1 + Int(max(textLabel.frame.width,14)) + marginLeft
        let tagWidth = textLeft1 + Int(max(textLabel.frame.width,14)) + 2 * marginLeft
